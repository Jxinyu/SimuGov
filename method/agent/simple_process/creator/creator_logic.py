...
def _watermark_tier(environment: Environment, watermark_id) -> str:
    if not watermark_id:
        return "中"
    lib = environment.watermark_technology_library.get("watermark_technology_library") or {}
    wk = lib.get(watermark_id)
    return wk["水印强度"] if wk else "中"
...
async def execute_group_creation_logic(
    ...
    for persona in agents:
        ...
        try:
            ...
            reason_str = f"基于群体策略(P={policy.post_probability})生成"
            ai_tool_tier = _watermark_tier(environment, watermark_id)
            log.info(
                f"\n[push_content]\n\tpersona=[agent_id={persona.agent_id}]\n\t"
                f"params=[reason={reason_str}\n\t\tcontent_type=image\n\t\t"
                f"topic={topic}\n\t\tcontent_detail={content_detail}\n\t\t"
                f"is_use_ai={is_use_ai}\n\t\tai_proportion={ai_proportion}\n\t\t"
                f"ai_tool_price_tier={ai_tool_tier}\n\t\tevasion={evasion}]\n\t"
...
async def add_new_content_to_environment(
    ...
    try:
        ...
        log.error(f"{persona.agent_id} 完整的堆栈跟踪信息如下:\n" + error_traceback)
...
async def _create_single_shadow_content(
    ...
    try:
        ...
        if base_prop is None:
            base_prop = 0
        ai_proportion = max(0.1, min(1.0, random.gauss(base_prop, settings.platform.ai_proportion_noise_std)))
        ...
        full_detail = f"[Shadow] {content_detail}"
        ai_tool_tier = _watermark_tier(environment, watermark_id)
        log.info(
            f"\n[push_content]\n\tpersona=[agent_id={persona.agent_id}]\n\t"
            f"params=[reason={reason}\n\t\tcontent_type={content_type}\n\t\t"
            f"topic={topic}\n\t\tcontent_detail={full_detail}\n\t\t"
            f"is_use_ai={is_use_ai}\n\t\tai_proportion={ai_proportion}\n\t\t"
            f"ai_tool_price_tier={ai_tool_tier}\n\t\tevasion={evasion}]\n\t"
            f"env=[day_time={environment.day_time}]")
