...
def generate_experimental_policies(policies_num=10) -> List[Policy]:
    """
    Randomly generate multiple sets of experimental policies for trend consistency verification.

    Returns:
        List[Policy]: A list containing the generated Policy objects.
    """
    policies1 = [
    ...
    policies3 = [Policy(ai_threshold=p['ai_threshold'], f_penalty=p['f_penalty'], e_edu=p['f_penalty']) for p in
                 population]
